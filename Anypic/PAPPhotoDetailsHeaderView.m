...
/*
 - (void)didTapLikeCommentButton:(UIButton *)button comment:(PFObject *)comment {
 
     // disable liking button temp
     [commentPhotoView shouldEnableLikeButton:NO];
     
     // change to selected
     BOOL liked = !button.selected;
     [commentButton setLikeStatus:liked];
     ...
     // increment count, update cache
     likeCount = [NSNumber numberWithInt:[likeCount intValue] + 1];
     [[PAPCache sharedCache] incrementLikerCountForPhoto:photo];
     } else {
     
     // decrement if unlike, update cache
     if ([likeCount intValue] > 0) {
     likeCount = [NSNumber numberWithInt:[likeCount intValue] - 1];
     }
     [[PAPCache sharedCache] decrementLikerCountForComment:photo];
     }
     
     [[PAPCache sharedCache] setCommentIsLikedByCurrentUser:photo liked:liked];
     ...
     // update like server-side
     [PAPUtility likeCommentInBackground:photo block:^(BOOL succeeded, NSError *error) {
     
     // PAPPhotoCommentView *actualHeaderView = (PAPPhotoHeaderView *)[self tableView:self.tableView viewForHeaderInSection:button.tag];
     ...
     [PAPUtility unlikeCommentInBackground:photo block:^(BOOL succeeded, NSError *error) {
